...
export default function Sidebar() {
  ...
  return (
    ...
    >
              ...
              <li key={item.name}>
                <NavLink
                  ...
                  className={({ isActive }) => cn(
                    "flex items-center py-1.5 px-2.5 rounded-md transition-all-200 text-sm",
                ...
                >
                  <item.icon size={18} className={cn("flex-shrink-0", !collapsed && "mr-2.5")} />
      ...
      <div className="border-t border-sidebar-border p-3">
        <div className={cn(
          ...
          <div className={collapsed ? "hidden" : "flex items-center"}>
            <BatteryMedium size={14} className="text-green-400 mr-2" />
      ...
      <div className="p-1.5 text-center text-xs text-gray-500 border-t border-sidebar-border">
        {!collapsed && (
          <div className="text-[10px]">© {new Date().getFullYear()} Anzo Controls</div>
        )}
      </div>
    </aside>
  );
